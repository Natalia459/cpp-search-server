...
public:
    ...
    void AddDocument(int document_id, const string& document) {
        ...
        ++document_count_;
        const double weight_of_word = 1. / words.size();
        for (const string& new_word : words) {
            documents_[new_word][document_id] += weight_of_word;
        }
    }
...
private:
    ...
    vector<Document> FindAllDocuments(const Query& query) const {
        ...
        for (const auto& [id, TF_IDF] : relevance) {
